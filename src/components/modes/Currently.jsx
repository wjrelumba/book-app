...
export default function Currently({books}) {
  ...
  // Get to read books
  const getBooks = () => {
    const booksRead = books.filter(book => book.currentRead);
  ...
  return (
    <div>
      {isLoading ? (
        ...
        (
          <>
          <h1 className='text-white text-xl text-center mb-2 font-mono uppercase bg-gray-800 rounded-2xl py-2'>Currently Reading</h1>
              ...
              {booksToRead.map((book) => (
                <div onClick={() => handleNavigation(book)} className='flex items-center justify-center w-full h-full py-1 mt-2 border-b-[2px] border-white' key={book.id}>
                  <div className='w-[60%] flex flex-col p-2'>
                    ...
                    <div className='mt-2'>
                    <Line percent={((book.currentPage/book.pageLength)*100)} strokeWidth={1} trailWidth={1}/>
